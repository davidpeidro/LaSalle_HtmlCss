...
{
    let PaisPrincipal = await getCountryInfo(queryForCountry);
    await delay(1000);
    let SegonPais = await getCountryInfo(queryForSegonCountry);
    let TercerPais = await getCountryInfo(queryForTercerCountry);
    let QuartPais = await getCountryInfo(queryForQuartCountry);

    /* Imaginem que no es repeteixen, ni cal comprovar-ho */
    console.log(PaisPrincipal);

    const divPaisos = document.getElementById("divPaisos")

    const pPregunta = document.createElement('p');
    pPregunta.innerText = "Quina es la capital de " + PaisPrincipal.data.country.name + "?";
    pPregunta.style.fontSize = "3em";
    pPregunta.display = "inline-flex";
    divPaisos.appendChild(pPregunta);

    const btPrincipal = document.createElement('p'); //input
    //btPrincipal.type = "button";
    btPrincipal.id = "btPrincipal";
    btPrincipal.innerText = PaisPrincipal.data.country.capital;
    //btPrincipal.style.fontSize = "3em";
    //btPrincipal.display = "inline-flex";
    divPaisos.appendChild(btPrincipal);

    const btSegonPais = document.createElement('p'); //input
    //btSegonPais.type = "button";
    btSegonPais.id = "btSegon";
    btSegonPais.innerText = SegonPais.data.country.capital;
    //btSegonPais.style.fontSize = "3em";
    //btSegonPais.display = "inline-flex";
    divPaisos.appendChild(btSegonPais);

    const btTercerPais = document.createElement('p'); //input
    //btTercerPais.type = "button";
    btTercerPais.id = "btTercer";
    btTercerPais.innerText = TercerPais.data.country.capital;
    //btTercerPais.style.fontSize = "3em";
    //btTercerPais.display = "inline-flex";
    divPaisos.appendChild(btTercerPais);

    const btQuartPais = document.createElement('p'); //input
    //btQuartPais.type = "button";
    btQuartPais.id = "btQuart";
    btQuartPais.innerText = QuartPais.data.country.capital;
    //btQuartPais.style.fontSize = "3em";
